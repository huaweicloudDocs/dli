...
1.  发送：**POST https://<IAM\_Endpoint\>/v3/auth/tokens**，请参见[地区和终端节点](https://developer.huaweicloud.com/dev/endpoint)，获取命令中IAM的Endpoint及消息体中的区域名称。
...
3.  调用业务API，在请求消息头中增加“X-Auth-Token”。
...
2.  <a name="li9829194965916"></a>工程中引入API网关签名SDK。
    1.  下载API网关签名工具。
    ...
    3.  将解压出来的jar文件引用到依赖路径中。如[图1](#fig1651517436011)所示：

        **图 1**  创建Java环境<a name="fig1651517436011"></a>  
        ![](figures/创建Java环境.png "创建Java环境")


3.  对请求进行签名。

    签名方法集成在[2](#li9829194965916)引入的jar文件中。发送请求前，需要对请求内容进行签名，得到的签名结果将作为http头部信息一起发送。
