# 用户指南

-   [简介](简介.md)
-   [准备工作](准备工作.md)
-   [入门](入门.md)
    -   [提交SQL作业](提交SQL作业.md)
    -   [提交Spark作业](提交Spark作业.md)

-   [总览](总览.md)
-   [SQL作业](SQL作业.md)
    -   [SQL作业总览](SQL作业总览.md)
    -   [SQL作业编辑器](SQL作业编辑器.md)
    -   [SQL数据管理](SQL数据管理.md)
        -   [概述](概述.md)
        -   [创建数据库和表](创建数据库和表.md)
        -   [删除数据库和表](删除数据库和表.md)
        -   [导入数据至DLI表](导入数据至DLI表.md)
            -   [通过OBS导入数据](通过OBS导入数据.md)
            -   [通过CDM导入数据](通过CDM导入数据.md)
            -   [通过DIS导入数据](通过DIS导入数据.md)

        -   [导入数据至OBS表](导入数据至OBS表.md)
        -   [将DLI数据导出至OBS](将DLI数据导出至OBS.md)
        -   [设置数据权限](设置数据权限.md)
        -   [查看元数据](查看元数据.md)
        -   [预览数据](预览数据.md)

    -   [SQL队列管理](SQL队列管理.md)
        -   [创建队列](创建队列.md)
        -   [删除队列](删除队列.md)
        -   [队列权限设置](队列权限设置.md)

    -   [SQL模板管理](SQL模板管理.md)
    -   [SQL作业管理](SQL作业管理.md)
    -   [设置数据权限和队列权限](设置数据权限和队列权限.md)
        -   [设置数据权限](设置数据权限-0.md)
        -   [队列权限设置](队列权限设置-1.md)


-   [Spark作业](Spark作业.md)
    -   [Spark作业总览](Spark作业总览.md)
    -   [Spark作业编辑器](Spark作业编辑器.md)
    -   [Spark集群管理](Spark集群管理.md)
        -   [创建集群](创建集群.md)
        -   [删除集群](删除集群.md)

    -   [Spark程序包管理](Spark程序包管理.md)
        -   [创建程序包](创建程序包.md)
        -   [删除程序包](删除程序包.md)

    -   [Spark作业管理](Spark作业管理.md)

-   [基因作业](基因作业.md)
    -   [基因作业总览](基因作业总览.md)
    -   [基因数据管理](基因数据管理.md)
    -   [基因测序](基因测序.md)
        -   [Germline Variant](Germline-Variant.md)
        -   [RNA差异表达分析](RNA差异表达分析.md)
        -   [体细胞突变检测](体细胞突变检测.md)

    -   [工具](工具.md)
    -   [帮助中心](帮助中心.md)

-   [提交Spark作业介绍](提交Spark作业介绍.md)
    -   [Spark作业功能介绍](Spark作业功能介绍.md)
    -   [创建Spark集群](创建Spark集群.md)
    -   [使用示例](使用示例.md)
    -   [使用DLI跨源能力](使用DLI跨源能力.md)
        -   [对接CloudTable](对接CloudTable.md)
            -   [CloudTable介绍](CloudTable介绍.md)
            -   [对接HBase](对接HBase.md)
                -   [通过公共API访问](通过公共API访问.md)

            -   [对接OpenTSDB](对接OpenTSDB.md)
                -   [通过公共API访问OpenTSDB](通过公共API访问OpenTSDB.md)
                -   [直连CloudTable集群访问（对等连接\)](直连CloudTable集群访问（对等连接).md)


        -   [对接RDS](对接RDS.md)
            -   [RDS介绍](RDS介绍.md)
            -   [使用SparkSQL方式对接RDS](使用SparkSQL方式对接RDS.md)
            -   [使用SQL方式对接RDS](使用SQL方式对接RDS.md)

        -   [对接DWS](对接DWS.md)
            -   [DWS介绍](DWS介绍.md)
            -   [使用SparkSQL方式对接DWS](使用SparkSQL方式对接DWS.md)
            -   [使用SQL方式对接DWS](使用SQL方式对接DWS.md)


    -   [集群已有程序包](集群已有程序包.md)
    -   [使用限制](使用限制.md)

-   [提交基因作业介绍](提交基因作业介绍.md)
    -   [基因作业功能介绍](基因作业功能介绍.md)
    -   [执行流程](执行流程.md)

-   [使用JDBC或ODBC连接DLI服务端](使用JDBC或ODBC连接DLI服务端.md)
    -   [下载JDBC或ODBC驱动包](下载JDBC或ODBC驱动包.md)
    -   [认证](认证.md)
    -   [使用JDBC连接服务端](使用JDBC连接服务端.md)
    -   [使用ODBC连接服务端](使用ODBC连接服务端.md)
    -   [JDBC API参考](JDBC-API参考.md)

-   [使用Beeline连接DLI服务端](使用Beeline连接DLI服务端.md)
    -   [DLI Beeline简介](DLI-Beeline简介.md)
    -   [DLI Beeline下载](DLI-Beeline下载.md)
    -   [使用DLI Beeline连接服务端](使用DLI-Beeline连接服务端.md)
    -   [DLI Beeline支持的命令](DLI-Beeline支持的命令.md)
    -   [DLI Beeline支持的命令行选项](DLI-Beeline支持的命令行选项.md)
    -   [查询输出格式](查询输出格式.md)

-   [永洪BI对接DLI操作指导](永洪BI对接DLI操作指导.md)
    -   [永洪BI对接准备工作](永洪BI对接准备工作.md)
    -   [永洪BI添加数据源](永洪BI添加数据源.md)
    -   [永洪BI创建数据集](永洪BI创建数据集.md)
    -   [永洪BI制作图表](永洪BI制作图表.md)

-   [Tableau Desktop对接DLI操作指导](Tableau-Desktop对接DLI操作指导.md)
    -   [Tableau Desktop对接准备工作](Tableau-Desktop对接准备工作.md)
    -   [Tableau Desktop添加数据源](Tableau-Desktop添加数据源.md)
    -   [Tableau Desktop创建数据集](Tableau-Desktop创建数据集.md)
    -   [Tableau Desktop制作图表](Tableau-Desktop制作图表.md)

-   [DLI支持地理空间查询](DLI支持地理空间查询.md)
    -   [地理空间查询基本概念](地理空间查询基本概念.md)
    -   [DLI支持的地理空间查询函数](DLI支持的地理空间查询函数.md)
        -   [DLI支持的地理空间查询函数分类](DLI支持的地理空间查询函数分类.md)
        -   [地理空间查询准备工作](地理空间查询准备工作.md)
        -   [地理构造函数](地理构造函数.md)
        -   [地理访问函数](地理访问函数.md)
        -   [地理转换函数](地理转换函数.md)
        -   [地理编辑函数](地理编辑函数.md)
        -   [地理输出函数](地理输出函数.md)
        -   [地理关系函数](地理关系函数.md)
        -   [地理处理函数](地理处理函数.md)


-   [通过云审计服务查看审计日志](通过云审计服务查看审计日志.md)
    -   [云审计服务支持的DLI操作列表](云审计服务支持的DLI操作列表.md)
    -   [查看审计日志](查看审计日志.md)

-   [调试环境使用](调试环境使用.md)

